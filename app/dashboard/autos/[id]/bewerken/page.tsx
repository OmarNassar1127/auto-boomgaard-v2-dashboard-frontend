...
export default function EditCarPage() {
  ...
  useEffect(() => {
    ...
    if (foundCar) {
      // Convert number values to strings for the form
      setCarData({
        brand: foundCar.brand,
        model: foundCar.model,
        price: foundCar.price.toString(),
        year: foundCar.year.toString(),
        mileage_km: foundCar.mileage_km.toString(),
        status: foundCar.status,
        is_favorite: foundCar.is_favorite,
        transmission: foundCar.transmission,
        fuel_type: foundCar.fuel_type,
        power_hp: foundCar.power_hp.toString(),
        engine: foundCar.engine,
        doors: foundCar.doors.toString(),
        seats: foundCar.seats.toString(),
        color: foundCar.color,
        body_type: foundCar.body_type,
        emissions_g_per_km: foundCar.emissions_g_per_km.toString(),
        energy_label: foundCar.energy_label,
        description: foundCar.description,
        maintenance_history: foundCar.maintenance_history,
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log({ ...carData, features, id: params.id })
      
      // Simulate API request
      setTimeout(() => {
        router.push(`/dashboard/autos/${params.id}`)
      }, 1000)
    } catch (error) {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <Header 
        title={`Auto bewerken: ${carData.brand} ${carData.model}`} 
        subtitle={`Auto ID: ${params.id}`}
      />
      ...
      <div className="flex-1 p-6 space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Bewerk de gegevens van deze auto
            </CardDescription>
          </CardHeader>
          <CardContent>
                ...
                <TabsContent value="features" className="space-y-6 mt-6">
                  <div className="space-y-4">
                    ...
                    <div className="border rounded-md p-4">
                      <h3 className="font-medium mb-3">Kenmerken</h3>
                      {features.length === 0 ? (
                  ...
                  <div className="border rounded-md p-4">
                    <h3 className="font-medium mb-3">Foto's</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                      {mockData.cars.find((car) => car.id === params.id)?.images.map((image, index) => (
                        <div key={index} className="aspect-video relative rounded-md overflow-hidden group">
                          <img
                            src={image}
                            alt={`Foto ${index + 1}`}
